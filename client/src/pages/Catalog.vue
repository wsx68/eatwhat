<template>
  <div class="catalog">
    ...
    <div class="filter-bar"><span v-for="loc in locations" :key="loc.v" class="chip" :class="{ on: curLoc === loc.v }" @click="switchLoc(loc.v)">{{ loc.label }}</span></div>
...
const q = ref(''); const lr = ref(null); const curLoc = ref('全部'); const curCat = ref('全部')
const locations = [{ v: '全部', label: '全部' },{ v: 'commercial', label: '🏪 商业街' },{ v: '一食堂', label: '一食堂' },{ v: '二食堂', label: '二食堂' },{ v: '三食堂', label: '三食堂' }]
...
async function loadData(reset) {
  ...
  try {
    ...
    if (j.code === 0) {
      let l = j.data.list
      if (curLoc.value !== '全部') l = l.filter(f => curLoc.value === 'commercial' ? f.restaurant_type === 'commercial' : f.location?.includes(curLoc.value))
...
function switchLoc(l) { curLoc.value = l; loadData(true) }
function switchCat(c) { curCat.value = c; loadData(true) }
function doSearch() { curLoc.value = '全部'; curCat.value = '全部'; if (!q.value.trim()) { loadData(true); return }; foodStore.search(q.value) }
